...
def assemble_mixed_field_t(
    ...
    for _ in range(n_sources):
        flux = float(torch.empty(1).uniform_(s_min, s_max, generator=rng))
        cx   = float(torch.empty(1).uniform_(lo, hi, generator=rng))
        cy   = float(torch.empty(1).uniform_(lo, hi, generator=rng))
        is_extended = float(torch.rand(1, generator=rng)) < extended_rate

        if is_extended:
            kind = int(torch.randint(0, 3, (1,), generator=rng))
            if kind == 0:
                sig_maj = float(torch.empty(1).uniform_(BEAM_SIGMA_PX, SIGMA_MAX_PX, generator=rng))
                sig_min = float(torch.empty(1).uniform_(BEAM_SIGMA_PX, sig_maj, generator=rng))
                pa      = float(torch.empty(1).uniform_(0.0, math.pi, generator=rng))
                sky += render_gaussian_blob_t(size, cx, cy, flux, sig_maj, sig_min, pa, device)
            elif kind == 1:
                radius    = float(torch.empty(1).uniform_(BEAM_SIGMA_PX, SIGMA_MAX_PX, generator=rng))
                thickness = float(torch.empty(1).uniform_(BEAM_SIGMA_PX * 0.5, max(BEAM_SIGMA_PX, radius * 0.5), generator=rng))
                sky += render_shell_t(size, cx, cy, flux, radius, thickness, device)
            else:
                length = float(torch.empty(1).uniform_(2 * BEAM_SIGMA_PX, 2 * SIGMA_MAX_PX, generator=rng))
                width  = float(torch.empty(1).uniform_(BEAM_SIGMA_PX, SIGMA_MAX_PX, generator=rng))
                pa     = float(torch.empty(1).uniform_(0.0, math.pi, generator=rng))
