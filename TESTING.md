...
validation protocol for all three variants. Read `design.md` §5–7 (A/B) and
§14 (C) for the algorithm specification before running end-to-end tests.
...
Expected output: **50 passed** in ~4–6 seconds on CPU. (Variant C tests pending — see §4.)
...
# 2. Simulate dirty/clean training pairs (one-time, all variants)
pixi run simulate

# 3. Train Variant A filter bank
...
```bash
pixi run simulate    # if not already done
pixi run train-conv
...
## 4. Variant C — Conditional Flow Matching (FlowTrainer + FlowSolver)

### 4.1 What it does

- **Training:** Conditional flow matching on dirty→clean pairs. The U-Net velocity
  field `v_θ` learns to map dirty islands to clean sky estimates via straight-line
  interpolation. No PSF is needed at inference — the PSF is implicit in the training
  data generated by `simulate_observations.py`.
- **Inference:** Euler ODE starting from the dirty island, integrating to t=1.
  n_samples perturbed trajectories give a per-pixel uncertainty estimate.

### 4.2 Training — step by step

```bash
# 1. Generate dirty/clean pairs (required)
pixi run simulate

# 2. Train (GPU required for full training)
pixi run -e gpu train-flow-gpu
# equivalent:
# python scripts/run_train.py --variant C \
#     --data crumb_data/flow_pairs.npz \
#     --out models/flow_model.pt \
#     --n_epochs 100 --batch_size 8 --lr 1e-4 --device cuda
```

Expected stdout per epoch:
```
FlowTrainer (dirty→clean): N=2100  150×150  batch=8  epochs=100  device=cuda
  Epoch   1/100  loss=1.234e-01
  Epoch   2/100  loss=7.429e-02
  ...
```

**What to check:**
- Loss decreases over epochs
- No NaN/Inf (indicates normalisation issue — check `flow_pairs.npz` was generated correctly)
...
Expected: `All smoke tests passed.` — covers data simulation, velocity field shapes,
FlowSolver shape assertions, and uncertainty key in deconvolver output.

### 4.4 Inference

```python
from mad_clean import FlowModel, FlowSolver, IslandDetector, MADClean
...
# Uncertainty map is populated (None for Variants A/B)
print(result["uncertainty"].shape)   # (H, W)
```

### 4.5 Validation protocol (Variant C)

Same residual RMS checks as Variants A/B, plus uncertainty inspection:

```python
import numpy as np
from astropy.io import fits

uncertainty = fits.getdata("results/mad_clean_C_Flow_uncertainty.fits")
model_C     = fits.getdata("results/mad_clean_C_Flow_model.fits")
dirty       = fits.getdata("dirty.fits")

# Uncertainty should be non-zero and spatially structured (higher near sources)
print(f"Uncertainty mean: {uncertainty.mean():.4e}")
print(f"Uncertainty max:  {uncertainty.max():.4e}")
assert uncertainty.max() > 0, "Uncertainty map is all-zero"
```

---

## 5. Head-to-Head Comparison (All Variants)

```python
import numpy as np
from astropy.io import fits

truth   = fits.getdata("crumb_data/source_truth.fits")
dirty   = fits.getdata("dirty.fits")
model_A = fits.getdata("results/mad_clean_A_Patch_model.fits")
model_B = fits.getdata("results/mad_clean_B_Conv_model.fits")
model_C = fits.getdata("results/mad_clean_C_Flow_model.fits")

def mse(a, b):
    return float(np.mean((np.squeeze(a) - np.squeeze(b)) ** 2))

print(f"MSE(dirty,   truth) = {mse(dirty,   truth):.4e}  [baseline]")
print(f"MSE(model_A, truth) = {mse(model_A, truth):.4e}  [Variant A]")
print(f"MSE(model_B, truth) = {mse(model_B, truth):.4e}  [Variant B]")
print(f"MSE(model_C, truth) = {mse(model_C, truth):.4e}  [Variant C]")
```

---

## 6. GPU Training (All Variants)

```bash
pixi run simulate                      # generate data (CPU, one-time)
...
- Variant A: minimal — runs on CPU
- Variant B (B=64, K=128, 150×150): ~4 GB VRAM
- Variant C (B=8, 150×150 UNet): ~2 GB VRAM

---

## 7. Resuming After a Session Break

```
1. Read design.md §1–4 (motivation, constraints, architecture)
2. Read plan.md Current Status table
3. Read TESTING.md §2, §3, or §4 for the relevant variant
